...
test('cli', async (t) => {
  ...
  t.test('--help', async (t) => {
    ...
    t.strictEqual(stdout, usage.trim(), 'output from --help')
  })
  ...
  t.test('failed lint', async (t) => {
    ...
    const {stderr, exitCode} = await execa.node(cli, ['-f', 'master'], {
      cwd
    }).catch((out) => {
      return out
    })
    ...
    t.match(stderr, /references may not be empty/gi)
  })
  ...
  t.test('successful lint', async (t) => {
    ...
    await execa.node(cli, ['-f', 'master'], {cwd}).catch((out) => {
